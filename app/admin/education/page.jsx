...
export default function AdminEducationPage() {
  ...
  useEffect(() => {
    async function fetchItems() {
      ...
      try {
        ...
        const data = snapshot.docs.map((d, idx) => ({ id: d.id, order: idx, ...d.data() }));
        setItems(data);
      } catch (err) {
  ...
  const validate = () => school.trim().length > 0 || degree.trim().length > 0;
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        ...
        setItems((prev) => prev.map((it) => (it.id === editingId ? { ...it, school, degree, startYear, endYear, description } : it)));
      } else {
        ...
        setItems((prev) => [...prev, { id: docRef.id, school, degree, startYear, endYear, description, order: prev.length }]);
      }
      setModalOpen(false);
      setEditingId(null);
    } catch (err) {
              ...
              <div ref={provided.innerRef} {...provided.droppableProps} className="space-y-4">
                {items.map((item, index) => (
                  <Draggable key={item.id || index} draggableId={String(item.id || index)} index={index}>
                    {(provided) => (
                      <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} className="card bg-base-100 shadow-sm p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div className="flex-1 grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                          {/* Degree */}
                          {inlineEditingId === item.id ? (
                            <input
                              value={item.degree}
                              onChange={(e) => setItems((prev) => prev.map((it) => (it.id === item.id ? { ...it, degree: e.target.value } : it)))}
                              onBlur={async () => {
                                setInlineEditingId(null);
                                try {
                                  if (item.id) await updateDoc(doc(db, "educations", item.id), { degree: item.degree });
                            ...
                            <span className="cursor-pointer" onClick={() => setInlineEditingId(item.id)}>{item.degree || "Degree"}</span>
                          )}
                          ...
                          {inlineEditingId === item.id ? (
                            <input
                              value={item.school}
                              onChange={(e) => setItems((prev) => prev.map((it) => (it.id === item.id ? { ...it, school: e.target.value } : it)))}
                              onBlur={async () => {
                                setInlineEditingId(null);
                                try {
                                  if (item.id) await updateDoc(doc(db, "educations", item.id), { school: item.school });
                                } catch (err) { console.error(err); }
                              }}
                              className="input input-bordered w-full"
                              placeholder="School"
                            ...
                            <span className="text-sm text-muted cursor-pointer" onClick={() => setInlineEditingId(item.id)}>{item.school || "School"}</span>
                          )}
                          ...
                          {inlineEditingId === item.id ? (
                            <input
                              ...
                              onChange={(e) => setItems((prev) => prev.map((it) => (it.id === item.id ? { ...it, startYear: e.target.value } : it)))}
                              onBlur={async () => {
                                setInlineEditingId(null);
                                try {
                                  if (item.id) await updateDoc(doc(db, "educations", item.id), { startYear: item.startYear ? Number(item.startYear) : null });
                                } catch (err) { console.error(err); }
                              }}
                              className="input input-bordered w-full"
                            ...
                            <span className="text-sm text-muted cursor-pointer" onClick={() => setInlineEditingId(item.id)}>{item.startYear || "—"}</span>
                          )}
                          ...
                          {inlineEditingId === item.id ? (
                            <input
                              value={item.endYear || ""}
                              onChange={(e) => setItems((prev) => prev.map((it) => (it.id === item.id ? { ...it, endYear: e.target.value } : it)))}
                              onBlur={async () => {
                                setInlineEditingId(null);
                                try {
                                  if (item.id) await updateDoc(doc(db, "educations", item.id), { endYear: item.endYear ? Number(item.endYear) : null });
                                } catch (err) { console.error(err); }
                              }}
                              className="input input-bordered w-full"
                            ...
                            <span className="text-sm text-muted cursor-pointer" onClick={() => setInlineEditingId(item.id)}>{item.endYear || "—"}</span>
                          )}
                        </div>

                        <div className="mt-4 md:mt-0 flex items-center gap-2">
                          <button className="btn btn-ghost btn-sm gap-2" onClick={() => setInlineEditingId(item.id)}>
          ...
          <div className="modal modal-open">
            <div className="modal-box max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="label"><span className="label-text">School</span></label>
                ...
                <div className="md:col-span-2">
                  <div className="flex items-center gap-4">
                    <img src={SAMPLE_IMAGE_URL} alt="preview" className="w-24 h-24 object-cover rounded-md shadow" onError={(e)=>{e.currentTarget.src=SAMPLE_IMAGE_URL}} />
                    <div>
              ...
              <div className="modal-action mt-4">
                <button className={`btn btn-primary ${saving ? "loading" : ""}`} onClick={handleSave}>{saving ? "Saving..." : editingId ? "Save Changes" : "Add Education"}</button>
